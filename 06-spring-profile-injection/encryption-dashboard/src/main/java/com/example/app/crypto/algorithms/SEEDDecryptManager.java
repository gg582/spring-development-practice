...
public class SEEDDecryptManager implements DecryptManager {
    ...
    public SEEDDecryptManager() {
        ...
        // KISA_SEED_CBC.SEED_CBC_init(seedInfo, KISA_ENC_DEC.KISA_DECRYPT, defaultKey, defaultIV);
        // Bean 설정을 통해 초기화하는 것이 더 좋습니다.
    }

    /**
     * @brief 데이터를 SEED 알고리즘으로 복호화합니다.
     *
     * @param data 복호화할 암호화된 데이터 바이트 배열.
    ...
    public byte[] decryptBytes(byte[] data, SecretKey key) throws Exception {
        ...
        // IV는 어디서 가져올 것인가? 현재는 기본값으로 설정하거나, 외부에서 주입받아야 함.
        ...
        return KISA_SEED_CBC.SEED_CBC_Decrypt(userKey, defaultIV, data, 0, data.length);
    }
    ...
    // 참고:
    // KISA_SEED_CBC.SEED_CBC_Decrypt 메소드가 내부적으로 필요한 초기화(SEED_CBC_init)를 수행하므로,
    // decryptBytes 메소드 내에서 직접 KISA_SEED_CBC.SEED_CBC_Decrypt를 호출하는 방식은,
